<template>
    <div class=" my-3">
        ...
        <h1 class="text-center mb-3">Demo #1</h1>
        ...
        <div class="my-4">
            <h5 class="text-center">Таблица оборудования</h5>
            <div class="mt-3 mb-1 d-flex justify-content-center ">
                <table class="border rounded border-shadow mb-1" style="min-width:500px;">
                    ...
                    <tr v-for="(value, index) in secrets.equipment" :key="index" class="p-2 border">
                        ...
                        <td class="text-secondary border p-2" style="max-width:300px;text-align: left;word-wrap: break-word;">{{ value.comment }}</td>
                    </tr>
                </table>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            setPagination() {
                ...
                for (let i = 0; i < this.secrets.meta.links.length; i++) {
                    let p = this.secrets.meta.links[i]
            ...
            editEquipment(){
              this.clearInfo() // remove delete info (if exist)
            },
            transferData(a,b,c,i) {
                // transfer data from table row into edit form
            ...
            search() {
                let word = this.searchEquipment.equipment.replace(/(<([^>]+)>)/gi, "")
                axios.get('/public/api/search/' + word)
                    .then(response=>{
